...
void LogixConfigDialog::setupUi()
{
  ...
  connect(rate_combo_, QOverload<int>::of(&QComboBox::currentIndexChanged), [this](int) {
    bool is_custom = (rate_combo_->currentData().toUInt() == 0);
    rate_custom_spin_->setVisible(is_custom);
    updateRamEstimate();
  });
  connect(rate_custom_spin_, QOverload<int>::of(&QSpinBox::valueChanged), this,
          &LogixConfigDialog::updateRamEstimate);
...
void LogixConfigDialog::populateTagTree(const std::vector<TagInfo>& tags)
{
  ...
  for (const auto& tag : tags)
  {
    ...
    if (tag.is_struct)
    {
      ...
      struct_item->setFlags(struct_item->flags() | Qt::ItemIsUserCheckable |
    ...
    else if (isNumericType(tag.symbol_type))
    {
      if (tag.array_dims > 0 && tag.array_size > 0)
      {
        ...
        array_item->setFlags(array_item->flags() | Qt::ItemIsUserCheckable |
                             Qt::ItemIsAutoTristate);
...
void LogixConfigDialog::onAccept()
{
  ...
  {
    QMessageBox::warning(this, "No Tags Selected", "Please select at least one tag to trend.");
    return;
  }
