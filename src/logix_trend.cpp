...
std::vector<TrendSample> TrendInstance::readData() {
    if (!running_ || instance_id_ == 0) return {};
...
uint8_t TrendInstance::deleteTrend() {
    ...
    auto resp = conn_.sendCip(msg);
    auto cip = parseCipStatus(resp);
    if (cip.status == 0) instance_id_ = 0;
    return cip.status;
}
...
double TrendInstance::interpretValue(const uint8_t* raw) const {
    switch (data_type_) {
        ...
        case kTypeINT: {
            int16_t v; std::memcpy(&v, raw, 2);
            return static_cast<double>(v);
        }
        case kTypeUINT: {
            uint16_t v; std::memcpy(&v, raw, 2);
            return static_cast<double>(v);
        }
        case kTypeDINT: {
