                                                     ...
                                                     uint16_t offset)
{
  ...
  if (!program_name.empty())
  {
    ...
    {
      path_segment.push_back(static_cast<uint8_t>(c));
    }
    if (program_name.size() % 2)
    {
      path_segment.push_back(0x00);
    }
  }
  ...
  while (!to_resolve.empty())
  {
    ...
    for (uint16_t type_id : to_resolve)
    {
      ...
      try
      {
        ...
        qDebug() << "UDT resolve: template" << type_id << "obj_def_size=" << obj_def_size
        ...
        if (tmpl_data.empty())
        {
          qDebug() << "UDT resolve: template" << type_id << "readTemplate returned empty (requested"
                   << size << "bytes)";
          continue;
        }
        ...
        if (tmpl_data.size() < member_defs_size)
        {
          continue;
        }
        ...
        if (!names.empty())
        {
          ...
          udt.name = (semicol != std::string::npos) ? names[0].substr(0, semicol) : names[0];
        }
      ...
      {
        qDebug() << "UDT resolve: template" << type_id << "exception:" << e.what();
        continue;
      }
    }
...
TagBrowser::expandStructMembers(const TagInfo& tag) const
{
