...
std::vector<uint8_t> buildSymbolicSegment(const std::string& tag_name)
{
  ...
  while (std::getline(ss, part, '.'))
  {
    ...
    for (char c : sym_name)
    {
      seg.push_back(static_cast<uint8_t>(c));
    }
    if (sym_name.size() % 2)
    {
      seg.push_back(0x00);
...
EipConnection::parseRouteString(const std::string& route_str)
{
  ...
  if (tokens.size() % 2 != 0)
  {
    throw std::runtime_error(
        "Invalid route: must have even number of comma-separated values (port,link pairs)");
  }
...
void EipConnection::connect(const std::string& ip,
                            const std::vector<std::pair<int, std::string>>& route, double timeout_s)
{
  close();
  route_ = route;
...
void EipConnection::registerSession()
{
  ...
  if (resp.size() < 24)
  {
    throw std::runtime_error("RegisterSession response too short");
  }
  uint32_t status = readU32(&resp[8]);
  if (status != 0)
  {
    throw std::runtime_error("RegisterSession failed");
  }
...
void EipConnection::forwardOpen()
{
  ...
  for (int attempt = 0; attempt < 2; attempt++)
  {
    ...
    auto buildFoHeader = [&](uint8_t service) {
      ...
      appendU8(fo, 0x20);
      appendU8(fo, 0x06);  // class 6
      appendU8(fo, 0x24);
      appendU8(fo, 0x01);  // instance 1
      appendU8(fo, 0x0A);
      appendU8(fo, 0x0E);  // priority/tick, timeout ticks
      appendU32(fo, ot_id);
      appendU32(fo, to_id);
    ...
    appendU16(payload, 0x00);
    appendU16(payload, 0);  // Null Address
    appendU16(payload, 0xB2);
    appendU16(payload, static_cast<uint16_t>(fo.size()));
    ...
    if (enip_status != 0)
    {
      throw std::runtime_error("Forward Open ENIP failed");
    }
    ...
    qDebug() << "ForwardOpen:" << (large_fo ? "Large(0x5B)" : "Standard(0x54)")
             << "fo_size=" << fo.size() << "cip_resp_size=" << cip_resp.size() << "cip_status="
             << (cip_resp.size() > 2 ? QString("0x%1").arg(cip_resp[2], 2, 16, QChar('0')) : "N/A");
    ...
    throw std::runtime_error("Forward Open CIP failed: 0x" + std::to_string(cip_status));
  }
...
std::vector<uint8_t> EipConnection::sendCip(const std::vector<uint8_t>& cip_message)
{
  ...
  if (resp.size() < 24)
  {
    throw std::runtime_error("SendUnitData response too short");
  }
  uint32_t status = readU32(&resp[8]);
  if (status != 0)
  {
    throw std::runtime_error("SendUnitData ENIP failed");
  }
...
std::vector<uint8_t> EipConnection::sendUnconnected(const std::vector<uint8_t>& cip_message)
{
  ...
  else
  {
    ...
    appendU8(frame, 0x20);
    appendU8(frame, 0x06);  // class 6
    appendU8(frame, 0x24);
    appendU8(frame, 0x01);  // instance 1
    appendU8(frame, 0x0A);
    appendU8(frame, 0x0E);  // priority/tick, timeout
    appendU16(frame, static_cast<uint16_t>(cip_message.size()));
    frame.insert(frame.end(), cip_message.begin(), cip_message.end());
    if (cip_message.size() % 2)
    {
      frame.push_back(0x00);  // pad
    }
  ...
  appendU16(payload, 0);  // Null Address
  appendU16(payload, 0xB2);
  appendU16(payload, static_cast<uint16_t>(frame.size()));
  ...
  if (resp.size() < 24)
  {
    throw std::runtime_error("SendRRData response too short");
  }
  uint32_t status = readU32(&resp[8]);
  if (status != 0)
  {
    throw std::runtime_error("SendRRData ENIP failed");
  }
...
void EipConnection::forwardClose()
{
  ...
  appendU16(payload, 0);
  appendU16(payload, 0xB2);
  appendU16(payload, static_cast<uint16_t>(fc.size()));
...
std::vector<uint8_t> EipConnection::parseSendRRResponse(const std::vector<uint8_t>& payload)
{
  if (payload.size() < 8)
  {
    throw std::runtime_error("SendRRData payload too short");
  }
  ...
  for (uint16_t i = 0; i < count; i++)
  {
    ...
    if (type_id == 0xB2)
    {
      return std::vector<uint8_t>(payload.begin() + offset, payload.begin() + offset + length);
    }
    offset += length;
  }
...
std::vector<uint8_t> EipConnection::parseSendUnitResponse(const std::vector<uint8_t>& payload)
{
  if (payload.size() < 8)
  {
    throw std::runtime_error("SendUnitData payload too short");
  }
  uint16_t count = readU16(&payload[6]);
  size_t offset = 8;
  for (uint16_t i = 0; i < count; i++)
  {
    ...
    if (type_id == 0xB1 && length >= 2)
    {
      ...
      return std::vector<uint8_t>(payload.begin() + offset + 2, payload.begin() + offset + length);
    }
    offset += length;
  }
  return {};
